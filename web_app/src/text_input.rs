...
fn get_value_from_text_area_event(e: InputEvent) -> String {
    let event: Event = e.dyn_into().unwrap_throw();
    let event_target = event.target().unwrap_throw();
...
pub fn get_value_from_select_event(event: Event) -> String {
    let event_target = event.target().unwrap_throw();
...
pub fn text_input(props: &Props) -> Html {
    ...
    let oninput = Callback::from(move |input_event: InputEvent| {
        on_change.emit(get_value_from_text_area_event(input_event));
    });

    html! {
        <textarea class="white-text" rows=5 {value} {oninput} />
    }
}
...
pub fn username_input(props: &Props) -> Html {
    ...
    html! {
        <input class="white-text" type="text" {value} {oninput} />
    }
}
